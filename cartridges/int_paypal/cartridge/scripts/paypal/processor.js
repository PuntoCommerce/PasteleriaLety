 ...
 */
function handle(basket, paymentInformation, req) {
    var {
        billingForm
    } = paymentInformation;
    ...
    const customFirstName = req.session.privacyCache.get("customerFirstName")
    const customerLastName = req.session.privacyCache.get("customerLastName")
    ...
    if (isBillingAgreementID) {
        ...
        updateBABillingAddress(basket, billing_info, customFirstName, customerLastName);
        ...
        updateOrderBillingAddress(basket, payer, customFirstName, customerLastName);
